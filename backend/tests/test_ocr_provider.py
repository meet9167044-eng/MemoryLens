...
# Simulated raw output that PaddleOCR v3 predict() would return
MOCK_PADDLE_OUTPUT = [
    {
        "dt_polys": [
            [[10, 20], [200, 20], [200, 40], [10, 40]],
            [[10, 50], [300, 50], [300, 70], [10, 70]],
            [[10, 80], [250, 80], [250, 100], [10, 100]]
        ],
        "rec_texts": [
            "RuntimeError: CUDA out of memory.",
            "Tried to allocate 2.00 GiB (GPU 0)",
...
def test_ocr_returns_correct_number_of_blocks(MockPaddleOCR):
    ...
    mock_engine.predict.return_value = MOCK_PADDLE_OUTPUT
...
def test_ocr_empty_image_returns_empty_blocks(MockPaddleOCR):
    ...
    mock_engine = MagicMock()
    mock_engine.predict.return_value = [{"dt_polys": [], "rec_texts": [], "rec_scores": []}]   # empty result
    MockPaddleOCR.return_value = mock_engine
...
def test_ocr_missing_paddleocr_returns_error():
    ...
    with patch("app.processing.ocr.provider.PaddleOCR", None), \
         patch("app.processing.ocr.provider._LOAD_ATTEMPTED", True):
